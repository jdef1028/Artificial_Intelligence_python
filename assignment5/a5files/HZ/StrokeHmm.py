...
class HMM:
    ...
    def label( self, data ):
        ...
        ##
        # #----------Comment Start for test case---------------------
        # # Part 1 Viterbi Testing Example
        # # (Test with the example from website as covered in class)
        # print 'This is a test case to illustrate viterbi algorithm as the example from the example website.'
        # # Input evidence: 'dry', 'damp', 'soggy'
        # data = [{'feature': 0}, {'feature': 2}, {'feature': 3}] 
        # # Define prior, transitions, and evidence probabilities
        # self.priors = {'sunny': 0.63, 'cloudy': 0.17, 'rainy': 0.20}
        # self.transitions = {'sunny': {'sunny':0.5, 'cloudy':0.375, 'rainy':0.125} , 'cloudy': {'sunny':0.25, 'cloudy':0.125, 'rainy':0.675}, 'rainy': {'sunny':0.25, 'cloudy':0.375, 'rainy':0.375}}
        # self.emissions = {'sunny': {'groundcond': [0.6, 0.2, 0.15, 0.05]}, 'cloudy': {'groundcond': [0.25, 0.25, 0.25, 0.25]}, 'rainy': {'groundcond': [0.05, 0.10, 0.35, 0.50]}} 
        # featureName = str('groundcond')
        # dataFeatureName = str('feature')
        # #----------Comment End for test case-----------------------------
        # #
        # Starting code for stroke recognition 
        # Initialize prior probabilities for each state 
        featureName = str('length')
        dataFeatureName = str('length')
        PriorProb = {}
        stateHistory = []
        for time in range(len(data)): 
            # Get number from feature (e.g., 1 is dry, 2 is dryish, 3 is damp, 4 is soggy)
            feature = data[time][dataFeatureName] 
            # Calculate current probability for each state based on prior probability 
            CurrentProb = {}
            for s1 in self.priors: # For each state, (e.g, 'sunny', 'cloudy', 'rainy')
                if time == 0:
                    # At t = 0, calculate current probability
                    CurrentProb[s1] = self.priors[s1]*self.emissions[s1][featureName][feature]
                else:
                    # At t > 0, update prior probability with current probability
                    S2Prob={}
                    for s2 in PriorProb: # For each state 'transitioned to'
                        S2Prob[s2] = PriorProb[s2]*self.emissions[s1][featureName][feature]*self.transitions[s1][s2]
                    # Find the maximum probability and state from each state (s2) as the most probable state of s1
                    MaxS2 = max(S2Prob.iteritems(), key=operator.itemgetter(1))[0]
                    CurrentProb[s1] = S2Prob[MaxS2]
            # Find the most probable state at time t
            MaxS1 = max(CurrentProb.iteritems(), key=operator.itemgetter(1))[0]
            PriorProb = copy.deepcopy(CurrentProb)
            stateHistory.append(MaxS1)
        # print 'Viterbi implemented.'
        # print 'According the classifier, final sequence of data has states: ', stateHistory
        # print 'Final probability of each state: ', PriorProb
        return stateHistory
...
class StrokeLabeler:
    ...
    def confusion(self, trueLabels, classifications):
        '''Takes truth labels and classifier output labels to construct a confusion matrix to illustrate classifier accuracy.'''
        if len(trueLabels) != len(classifications):
            print 'Length of truth and classification labels are inconsistent! Quitting now...'
            return None
        TrueDrawing = 0 # get from trueLabels
        TrueText = 0 
        TruePositiveDrawing = 0 # get from comparison between truth and classifications
        TruePositiveText = 0
        for numOflabel in range(len(trueLabels)):
            truelabel = trueLabels[numOflabel]
            label = classifications[numOflabel]
            if truelabel == 'drawing':
                TrueDrawing += 1
                if label == 'drawing':
                    TruePositiveDrawing += 1
            elif truelabel == 'text':
                TrueText += 1
                if label == 'text':
                    TruePositiveText += 1
        ...
        D1 = {}
        D2 = {}
        # ConfusionMatrix['drawing']['drawing'] = TruePositiveDrawing
        D1['drawing'] = TruePositiveDrawing
        # ConfusionMatrix['drawing']['text'] = TrueDrawing - TruePositiveDrawing
        D1['text'] = TrueDrawing - TruePositiveDrawing
        # ConfusionMatrix['text']['text'] = TruePositiveText
        D2['text'] = TruePositiveText
        # ConfusionMatrix['text']['drawing'] = TrueText - TruePositiveText
        D2['drawing'] = TrueText - TruePositiveText
        # print D1
        # print D2
        ConfusionMatrix['drawing'] = D1
        ConfusionMatrix['text'] = D2
        return ConfusionMatrix

    def findConfusionData(self):
        lFileList = [] # Get all files from the training files folder
        for fFileObj in os.walk("../trainingFiles/"):
        ...
        for fn in lFileList:
            ct += 1
            print 'filename:', '../trainingFiles/'+fn
            strokes, truelabels = self.loadLabeledFile('../trainingFiles/'+fn)
            predictLabels = self.labelStrokes(strokes)
            for numOflabel in range(len(truelabels)):
                truelabel = truelabels[numOflabel]
                lTrueLabels.append(truelabel)
                label = predictLabels[numOflabel]
                lPredictLabels.append(label)
            # if ct > 5:
            #     break                               
        return lTrueLabels, lPredictLabels
