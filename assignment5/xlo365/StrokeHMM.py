...
class HMM:
	...
	def trainPriors(self, trainingData, trainingLabels):
		...
		for labels in trainingLabels:
			priorCounts[labels[0]] += 1 #count number
		...
		for labels in trainingLabels:
			if len(labels) > 1:
				...
				for lab2 in labels[1:]:
					transitionCounts[lab1][lab2] += 1 #lab 1 is the former one, lab2 is the later one
					lab1 = lab2
			...
			for f in featureVals[s].keys():
				...
				if self.featuresCorD[f] == DISCRETE:
					...
					for i in range(len(self.emissions[s][f])):
						self.emissions[s][f][i] /= float(len(featureVals[s][f]) + self.numVals[f]) #normalization
	...
	def label(self, data):
		...
		for num in range(1, len(data)):
			for s2 in self.states:  # current state
				...
				for s1 in self.states:  # previous state
					...
					# calculate the final probability by multiplying the three probability components together
		...
		self.sequence = []
		for i in range(len(data)):
			temp = sorted(self.probs[i].items(), key=lambda x: x[1]) # sort the dict based on the probability

			self.sequence.append(temp[-1][0]) # find the most likely one
...
# =========================================================
# #execfile("StrokeHmm.py") #only needed in the separate file
...
class StrokeLabeler:
	def __init__(self):
		...
		self.featureNames = ['length', "curvature", "H_W_ratio",'boxArea']
		self.contOrDisc = {'length': DISCRETE, "curvature": CONTINUOUS, "H_W_ratio": DISCRETE, 'boxArea':DISCRETE}
		self.numFVals = {'length': 2, "H_W_ratio": 2, 'boxArea':2}
	...
	def confusion(self, trueLabels, classification):
		...
		print count
		Percent_correct = {}
		Percent_correct["drawing"] = float(count["drawing"]["drawing"]) / float(
			count["drawing"]["drawing"] + count["drawing"]["text"])
	...
	def featureTest(self, strokeFile):
		...
		for i in range(len(strokes)):
			...
			print "HeightWidth Ratio is", strokes[i].HeightWidthRatio()
			print "boxArea is", strokes[i].boxArea()
...
class Stroke:
	...
	def HeightWidthRatio(self):
		xs = []
		ys = []
		for ele in self.points:
			xs.append(ele[0])
			ys.append(ele[1])
		width = float(max(xs)) - float(min(xs))
		height = float(max(ys)) - float(min(ys))
		return float(height) / (float(width) + 0.0001)

	def boxArea(self):

		'''Returns bounding box area of the stroke '''
		BIGNUMBER = 1E6
		maxX = 0
		maxY = 0
		minX = BIGNUMBER
...
#===================================================================
#   Testing code for 2 & 3 ends here
#====================================================================
